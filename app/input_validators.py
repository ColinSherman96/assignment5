...
class InputValidator:
    ...
    def validate_number(value: Any, config: CalculatorConfig) -> Decimal:
        ...
        Raises:
            OperationError: If input is invalid
        """
        try:
            ...
            if abs(number) > config.max_input_value:
                raise OperationError(f"Value exceeds maximum allowed: {config.max_input_value}")
            return number.normalize()
        except InvalidOperation as e:
            raise OperationError(f"Invalid number format: {value}") from e