...
def test_validate_number_invalid_string():
    with pytest.raises(OperationError, match="Invalid number format: abc"):
        InputValidator.validate_number("abc", config)

def test_validate_number_exceeds_max_value():
    with pytest.raises(OperationError, match="Value exceeds maximum allowed"):
...
def test_validate_number_empty_string():
    with pytest.raises(OperationError, match="Invalid number format: "):
...
def test_validate_number_none_value():
    with pytest.raises(OperationError, match="Invalid number format: None"):
